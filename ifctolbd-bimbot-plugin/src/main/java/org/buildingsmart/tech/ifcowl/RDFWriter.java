...
public class RDFWriter {
	...
	public RDFWriter(OntModel ontModel, OntModel expressModel, OntModel listModel, InputStream inputStream,
			String baseURI, Map<String, EntityVO> ent, Map<String, TypeVO> typ, String ontURI) {
	...
	private void resolveDuplicates() throws IOException {
		...
		if (myIfcReaderStream.logToFile)
			myIfcReaderStream.bw
					.write("MESSAGE: found and removed " + listOfDuplicateLineEntries.size() + " duplicates! \r\n");
			...
			for (int i = 0; i < vo.getObjectList().size(); i++) {
				...
				if (Character.class.isInstance(o)) {
					if ((Character) o != ',') {
						if (myIfcReaderStream.logToFile)
							myIfcReaderStream.bw.write(
									"*ERROR 15*: We found a character that is not a comma. That should not be possible"
											+ "\r\n");
					}
					...
					if (s.charAt(0) == '#') {
						...
						if (or == null) {
							if (myIfcReaderStream.logToFile)
								myIfcReaderStream.bw.write("*ERROR 6*: Reference to non-existing line number in line: #"
										+ vo.getLineNum() + "=" + vo.getFullLineAfterNum()
										+ "\r\nQuitting the application without output!\r\n ");
							System.err.println("*ERROR 6*: Reference to non-existing line number in line: #"
									+ vo.getLineNum() + " - " + vo.getFullLineAfterNum()
									+ "\r\nQuitting the application without output!");
							return false;
						}
						...
						if (Character.class.isInstance(o)) {
							...
							if (s.charAt(0) == '#') {
								...
								if (or == null) {
									if (myIfcReaderStream.logToFile)
										myIfcReaderStream.bw
												.write("*ERROR 7*: Reference to non-existing line number in line: #"
														+ vo.getLineNum() + "=" + vo.getFullLineAfterNum()
														+ "\r\nQuitting the application without output!\r\n ");
									System.err.println("*ERROR 7*: Reference to non-existing line number in line: #"
											+ vo.getLineNum() + " - " + vo.getFullLineAfterNum()
											+ "\r\nQuitting the application without output!");
									tmpList.set(j, "-");
									return false;
								...
								if (String.class.isInstance(o2)) {
									...
									if (s.charAt(0) == '#') {
										...
										if (or == null) {
											if (myIfcReaderStream.logToFile)
												myIfcReaderStream.bw.write(
														"*ERROR 8*: Reference to non-existing line number in line: #"
																+ vo.getLineNum() + "=" + vo.getFullLineAfterNum()
																+ "\r\nQuitting the application without output!\r\n ");
											System.err.println(
													"*ERROR 8*: Reference to non-existing line number in line: #"
	...
	private void fillProperties(IFCVO ifcLineEntry, Resource r, OntClass cl) throws IOException {
		...
		if (tvo == null && evo == null) {
			...
			if (myIfcReaderStream.logToFile)
				myIfcReaderStream.bw.write(
						"*ERROR 3*: fillProperties 1 - Type nor entity exists: " + ifcLineEntry.getName() + "\r\n");
			...
			for (Object o : ifcLineEntry.getObjectList()) {
				...
				if (Character.class.isInstance(o)) {
					if ((Character) o != ',') {
						if (myIfcReaderStream.logToFile)
							myIfcReaderStream.bw.write(
									"*ERROR 17*: We found a character that is not a comma. That should not be possible!"
											+ "\r\n");
					}
				} else if (String.class.isInstance(o)) {
					if (myIfcReaderStream.logToFile)
						myIfcReaderStream.bw.write(
								"*WARNING 1*: fillProperties 2 - WARNING: unhandled type property found." + "\r\n");
					System.out.println("*WARNING 1*: unhandled type property found.");
				} else if (IFCVO.class.isInstance(o)) {
					if (myIfcReaderStream.logToFile)
						myIfcReaderStream.bw.write(
								"*WARNING 2*: fillProperties 2 - WARNING: unhandled type property found." + "\r\n");
			...
			for (Object o : ifcLineEntry.getObjectList()) {
				...
				if (Character.class.isInstance(o)) {
					if ((Character) o != ',') {
						if (myIfcReaderStream.logToFile)
							myIfcReaderStream.bw.write(
									"*ERROR 18*: We found a character that is not a comma. That should not be possible!"
											+ "\r\n");
					}
				} else if (String.class.isInstance(o)) {
					if (myIfcReaderStream.logToFile)
			...
			if (typ.get(ExpressReader.formatClassName((String) o)) == null) {
				if ((evo != null) && (evo.getDerivedAttributeList() != null)) {
					if (evo.getDerivedAttributeList().size() <= attributePointer) {
						if (myIfcReaderStream.logToFile)
							myIfcReaderStream.bw.write(
									"*ERROR 4*: Entity in IFC files has more attributes than it is allowed have: "
											+ subject + "\r\n");
						System.err
								.println("*ERROR 4*: Entity in IFC files has more attributes than it is allowed have: "
										+ subject);
						attributePointer++;
					...
					if (range.isClass()) {
						if (range.asClass().hasSuperClass(expressModel.getOntClass(expressNS + "ENUMERATION"))) {
							...
							if (myIfcReaderStream.logToFile)
								myIfcReaderStream.bw
										.write("*OK 25*: found subClass of SELECT Class, now doing nothing with it: "
							...
							if (myIfcReaderStream.logToFile)
								myIfcReaderStream.bw
										.write("*WARNING 5*: found LIST property (but doing nothing with it): "
												+ subject + " -- " + p + " - " + range.getLocalName() + " - "
												+ literalString + "\r\n");
						} else {
							createLiteralProperty(r, p, range, literalString);
						}
						...
						if (myIfcReaderStream.logToFile)
							myIfcReaderStream.bw.write("*WARNING 7*: found other kind of property: " + p + " - "
									+ range.getLocalName() + "\r\n");
					}
				} else {
					if (myIfcReaderStream.logToFile)
						myIfcReaderStream.bw.write(
								"*WARNING 8*: Nothing happened. Not sure if this is good or bad, possible or not."
										+ "\r\n");
				}
				attributePointer++;
			} else {
	...
	private int fillPropertiesHandleIfcObject(Resource r, EntityVO evo, int attributePointer, Object o)
		...
		if ((evo != null) && (evo.getDerivedAttributeList() != null)
			...
			if (myIfcReaderStream.logToFile)
				myIfcReaderStream.bw.write("*OK 1*: added property: " + r.getLocalName() + " - " + p.getLocalName()
						+ " - " + r1.getLocalName() + "\r\n");
		} else {
			if (myIfcReaderStream.logToFile)
				myIfcReaderStream.bw.write(
						"*WARNING 3*: Nothing happened. Not sure if this is good or bad, possible or not." + "\r\n");
		}
		attributePointer++;
		return attributePointer;
	}
	...
	private int fillPropertiesHandleListObject(Resource r, EntityVO evo, int attributePointer, Object o)
		...
		for (int j = 0; j < tmpList.size(); j++) {
			...
			if (Character.class.isInstance(o1)) {
				...
				if (c != ',') {
					if (myIfcReaderStream.logToFile)
						myIfcReaderStream.bw.write(
								"*ERROR 12*: We found a character that is not a comma. That is odd. Check!" + "\r\n");
				}
			} else if (String.class.isInstance(o1)) {
				...
				if (typeRemembrance == null) {
					...
					if (t != null) {
						if (t == typeRemembrance) {
							...
							if (myIfcReaderStream.logToFile)
								myIfcReaderStream.bw.write(
										"*WARNING 37*: Found two different types in one list. This is worth checking.\r\n ");
						}
					} else {
						literals.add(filterExtras((String) o1));
					}
				...
				if ((evo != null) && (evo.getDerivedAttributeList() != null)
					...
					if (typerange.asClass().hasSuperClass(listModel.getOntClass(listNS + "OWLList"))) {
						...
						ttlWriter.triple(new Triple(r.asNode(), p.asNode(), r1.asNode()));
						if (myIfcReaderStream.logToFile)
							myIfcReaderStream.bw.write("*OK 5*: added property: " + r.getLocalName() + " - "
									+ p.getLocalName() + " - " + r1.getLocalName() + "\r\n");
					}
				} else {
					if (myIfcReaderStream.logToFile)
						myIfcReaderStream.bw.write(
								"*WARNING 13*: Nothing happened. Not sure if this is good or bad, possible or not."
										+ "\r\n");
				}
			} else if (LinkedList.class.isInstance(o1)) {
				if (typeRemembrance != null) {
					...
					for (int jj = 0; jj < tmpListInList.size(); jj++) {
						...
						if (Character.class.isInstance(o2)) {
							...
							if (myIfcReaderStream.logToFile)
								myIfcReaderStream.bw.write(
										"*ERROR 19*: Found List of List of List. Code cannot handle that." + "\r\n");
					...
					if ((evo != null) && (evo.getDerivedAttributeList() != null)
						...
						if (typerange.asClass().hasSuperClass(listModel.getOntClass(listNS + "OWLList"))) {
							...
							if (myIfcReaderStream.logToFile)
								myIfcReaderStream.bw.write(
										"*ERROR 23*: Impossible: found a list that is actually not a list." + "\r\n");
							System.err.println(
									"*ERROR 23: Impossible: found a list that is actually not a list." + "\r\n");
						}
					}
		...
		if (literals.size() > 0) {
			...
			if (typeRemembrance != null) {
				if ((evo != null) && (evo.getDerivedAttributeList() != null)
					...
					if (typerange.asClass().hasSuperClass(listModel.getOntClass(listNS + "OWLList")))
						...
						if (literals.size() > 1) {
							if (myIfcReaderStream.logToFile)
								myIfcReaderStream.bw.write(
										"*WARNING 37*: We are ignoring a number of literal values here." + "\r\n");
						}
					}
				} else {
					if (myIfcReaderStream.logToFile)
						myIfcReaderStream.bw.write(
								"*WARNING 15*: Nothing happened. Not sure if this is good or bad, possible or not."
										+ "\r\n");
				}
				typeRemembrance = null;
				...
				if (myIfcReaderStream.logToFile)
					myIfcReaderStream.bw
							.write("*WARNING 14*: Nothing happened. Not sure if this is good or bad, possible or not."
									+ "\r\n");
			}
		}
		...
		for (int j = 0; j < tmpList.size(); j++) {
			...
			if (Character.class.isInstance(o1)) {
				...
				if (c != ',') {
					if (myIfcReaderStream.logToFile)
						myIfcReaderStream.bw.write(
								"*ERROR 13*: We found a character that is not a comma. That is odd. Check!" + "\r\n");
				}
			} else if (String.class.isInstance(o1)) {
				...
				if ((tvo != null)) {
					...
					if (myIfcReaderStream.logToFile)
						myIfcReaderStream.bw.write(
								"*WARNING 19*: Nothing happened. Not sure if this is good or bad, possible or not."
										+ "\r\n");
				}
				...
				for (int jj = 0; jj < tmpListInlist.size(); jj++) {
					...
					if (String.class.isInstance(o2)) {
						...
						if (myIfcReaderStream.logToFile)
							myIfcReaderStream.bw.write(
									"*WARNING 18*: Nothing happened. Not sure if this is good or bad, possible or not."
											+ "\r\n");
					}
				}
			} else {
		...
		if (literals.size() > 0) {
			if (typeRemembrance != null) {
				if ((tvo != null)) {
					...
					if (myIfcReaderStream.logToFile)
						myIfcReaderStream.bw
								.write("*WARNING 20*: this part of the code has not been checked - it can't be correct"
										+ "\r\n");
					...
					if (myIfcReaderStream.logToFile)
						myIfcReaderStream.bw.write(
								"*WARNING 21*: Nothing happened. Not sure if this is good or bad, possible or not."
										+ "\r\n");
				}
				typeRemembrance = null;
			} else if ((tvo != null)) {
	...
	private void addSinglePropertyFromTypeRemembrance(Resource r, OntProperty p, String literalString,
		...
		if (range.isClass()) {
			if (range.asClass().hasSuperClass(expressModel.getOntClass(expressNS + "ENUMERATION"))) {
				...
				// Check for SELECT
				if (myIfcReaderStream.logToFile)
					myIfcReaderStream.bw.write("*OK 24*: found subClass of SELECT Class, now doing nothing with it: "
							+ p + " - " + range.getLocalName() + " - " + literalString + "\r\n");
				createLiteralProperty(r, p, range, literalString);
			} else if (range.asClass().hasSuperClass(listModel.getOntClass(listNS + "OWLList"))) {
				// Check for LIST
				if (myIfcReaderStream.logToFile)
					myIfcReaderStream.bw.write("*WARNING 24*: found LIST property (but doing nothing with it): " + p
			...
			if (myIfcReaderStream.logToFile)
				myIfcReaderStream.bw.write(
						"*WARNING 26*: found other kind of property: " + p + " - " + range.getLocalName() + "\r\n");
		}
	}
	...
	private void addEnumProperty(Resource r, Property p, OntResource range, String literalString) throws IOException {
		for (ExtendedIterator<? extends OntResource> instances = range.asClass().listInstances(); instances
			...
			if (rangeInstance.getProperty(RDFS.label).getString().equalsIgnoreCase(filterPoints(literalString))) {
				...
				if (myIfcReaderStream.logToFile)
					myIfcReaderStream.bw.write("*OK 2*: added ENUM statement " + r.getLocalName() + " - "
							+ p.getLocalName() + " - " + rangeInstance.getLocalName() + "\r\n");
				return;
			}
		}
		if (myIfcReaderStream.logToFile)
	...
	private void addLiteralToResource(Resource r1, OntProperty valueProp, String xsdType, String literalString)
		...
		if (myIfcReaderStream.logToFile)
			myIfcReaderStream.bw.write(
					"*OK 4*: added literal: " + r1.getLocalName() + " - " + valueProp + " - " + literalString + "\r\n");
	}
			...
			if (listrange.asClass().hasSuperClass(listModel.getOntClass(listNS + "OWLList"))) {
				...
				if (mySwitch == 0) {
					...
					for (int i = 0; i < reslist.size(); i++) {
						...
						OntResource rclass = ontModel.getOntResource(ontNS + evorange.getName());
						Resource r2 = getResource(baseURI + evorange.getName() + "_" + ((IFCVO) vo).getLineNum(),
								rclass);
						if (myIfcReaderStream.logToFile)
							myIfcReaderStream.bw.write("*OK 21*: created resource: " + r2.getLocalName() + "\r\n");
						IDcounter++;
						ttlWriter.triple(new Triple(r1.asNode(),
								listModel.getOntProperty(listNS + "hasContents").asNode(), r2.asNode()));
						if (myIfcReaderStream.logToFile)
							myIfcReaderStream.bw.write("*OK 22*: added property: " + r1.getLocalName() + " - "
									+ "-hasContents-" + " - " + r2.getLocalName() + "\r\n");

						if (i < el.size() - 1) {
							ttlWriter.triple(
									new Triple(r1.asNode(), listModel.getOntProperty(listNS + "hasNext").asNode(),
											reslist.get(i + 1).asNode()));
							if (myIfcReaderStream.logToFile)
								myIfcReaderStream.bw.write("*OK 23*: added property: " + r1.getLocalName() + " - "
										+ "-hasNext-" + " - " + reslist.get(i + 1).getLocalName() + "\r\n");
						}
					}
				}
			}
	...
	private void addRegularListProperty(Resource r, OntProperty p, List<String> el, TypeVO typeRemembranceOverride)
		...
		if (range.isClass()) {
			...
			if (listrange == null) {
				if (myIfcReaderStream.logToFile)
					myIfcReaderStream.bw.write(
							"*ERROR 14*: We could not find what kind of content is expected in the LIST." + "\r\n");
			} else {
				if (listrange.asClass().hasSuperClass(listModel.getOntClass(listNS + "OWLList"))) {
					if (myIfcReaderStream.logToFile)
					...
					for (int ii = 0; ii < el.size(); ii++) {
						...
						if (ii == 0) {
							ttlWriter.triple(new Triple(r.asNode(), p.asNode(), r1.asNode()));
							if (myIfcReaderStream.logToFile)
								myIfcReaderStream.bw.write("*OK 7*: added property: " + r.getLocalName() + " - "
										+ p.getLocalName() + " - " + r1.getLocalName() + "\r\n");
						}
					}
					// bindtheproperties
	...
	private void createLiteralProperty(Resource r, OntResource p, OntResource range, String literalString)
		...
		if (xsdType != null) {
			...
			ttlWriter.triple(new Triple(r.asNode(), p.asNode(), r1.asNode()));
			if (myIfcReaderStream.logToFile)
				myIfcReaderStream.bw.write("*OK 3*: added property: " + r.getLocalName() + " - " + p.getLocalName()
						+ " - " + r1.getLocalName() + "\r\n");
		} else {
			if (myIfcReaderStream.logToFile)
				myIfcReaderStream.bw.write("*ERROR 1*: XSD type not found for: " + p + " - " + range.getURI() + " - "
						+ literalString + "\r\n");
		}
	}
		...
		for (int i = 0; i < tmpList.size(); i++) {
			if (IFCVO.class.isInstance(tmpList.get(i))) {
				...
				if (i == 0) {
					ttlWriter.triple(new Triple(r.asNode(), p.asNode(), r1.asNode()));
					if (myIfcReaderStream.logToFile)
						myIfcReaderStream.bw.write("*OK 13*: added property: " + r.getLocalName() + " - "
								+ p.getLocalName() + " - " + r1.getLocalName() + "\r\n");
				}
			}
		}
	...
	private void addClassInstanceListProperties(List<Resource> reslist, List<IFCVO> entlist, OntResource listrange)
		...
		for (int i = 0; i < reslist.size(); i++) {
			...
			if (evorange == null) {
				...
				if (myIfcReaderStream.logToFile)
					myIfcReaderStream.bw.write("*OK 8*: created property: " + r.getLocalName() + " - "
							+ listp.getLocalName() + " - " + r1.getLocalName() + "\r\n");
			} else {
				...
				ttlWriter.triple(new Triple(r.asNode(), listp.asNode(), r1.asNode()));
				if (myIfcReaderStream.logToFile)
					myIfcReaderStream.bw.write("*OK 9*: created property: " + r.getLocalName() + " - "
							+ listp.getLocalName() + " - " + r1.getLocalName() + "\r\n");
			}
			...
			if (i < reslist.size() - 1) {
				...
				if (myIfcReaderStream.logToFile)
					myIfcReaderStream.bw.write("*OK 10*: created property: " + r.getLocalName() + " - "
							+ isfollowed.getLocalName() + " - " + reslist.get(i + 1).getLocalName() + "\r\n");
			}
		}
	}
	...
	private void addListInstanceProperties(List<Resource> reslist, List<String> listelements, OntResource listrange)
		...
		if (xsdType != null) {
			...
			for (int i = 0; i < reslist.size(); i++) {
				...
				ttlWriter.triple(
						new Triple(r.asNode(), listModel.getOntProperty(listNS + "hasContents").asNode(), r2.asNode()));
				if (myIfcReaderStream.logToFile)
					myIfcReaderStream.bw.write("*OK 11*: added property: " + r.getLocalName() + " - " + "-hasContents-"
							+ " - " + r2.getLocalName() + "\r\n");

				if (i < listelements.size() - 1) {
					ttlWriter.triple(new Triple(r.asNode(), listModel.getOntProperty(listNS + "hasNext").asNode(),
							reslist.get(i + 1).asNode()));
					if (myIfcReaderStream.logToFile)
						myIfcReaderStream.bw.write("*OK 12*: added property: " + r.getLocalName() + " - " + "-hasNext-"
	...
	private OntResource getListContentType(OntClass range) throws IOException {
		if (range.asClass().getURI().equalsIgnoreCase(expressNS + "STRING_List")
				|| range.asClass().hasSuperClass(expressModel.getOntClass(expressNS + "STRING_List")))
			return expressModel.getOntResource(expressNS + "STRING");
		else if (range.asClass().getURI().equalsIgnoreCase(expressNS + "REAL_List")
				|| range.asClass().hasSuperClass(expressModel.getOntClass(expressNS + "REAL_List")))
			return expressModel.getOntResource(expressNS + "REAL");
		else if (range.asClass().getURI().equalsIgnoreCase(expressNS + "INTEGER_List")
				|| range.asClass().hasSuperClass(expressModel.getOntClass(expressNS + "INTEGER_List")))
			return expressModel.getOntResource(expressNS + "INTEGER");
		else if (range.asClass().getURI().equalsIgnoreCase(expressNS + "BINARY_List")
				|| range.asClass().hasSuperClass(expressModel.getOntClass(expressNS + "BINARY_List")))
			return expressModel.getOntResource(expressNS + "BINARY");
		else if (range.asClass().getURI().equalsIgnoreCase(expressNS + "BOOLEAN_List")
				|| range.asClass().hasSuperClass(expressModel.getOntClass(expressNS + "BOOLEAN_List")))
			return expressModel.getOntResource(expressNS + "BOOLEAN");
		else if (range.asClass().getURI().equalsIgnoreCase(expressNS + "LOGICAL_List")
				|| range.asClass().hasSuperClass(expressModel.getOntClass(expressNS + "LOGICAL_List")))
			return expressModel.getOntResource(expressNS + "LOGICAL");
		else if (range.asClass().getURI().equalsIgnoreCase(expressNS + "NUMBER_List")
				|| range.asClass().hasSuperClass(expressModel.getOntClass(expressNS + "NUMBER_List")))
			...
			if (myIfcReaderStream.logToFile) {
				myIfcReaderStream.bw
						.write("*WARNING 29*: did not find listcontenttype for : " + range.getLocalName() + "\r\n");
			}
			return null;
		}
	}
	...
	private String getXSDTypeFromRange(OntResource range) {
		if (range.asClass().getURI().equalsIgnoreCase(expressNS + "STRING")
				|| range.asClass().hasSuperClass(expressModel.getOntClass(expressNS + "STRING")))
			return "string";
		else if (range.asClass().getURI().equalsIgnoreCase(expressNS + "REAL")
				|| range.asClass().hasSuperClass(expressModel.getOntClass(expressNS + "REAL")))
			return "double";
		else if (range.asClass().getURI().equalsIgnoreCase(expressNS + "INTEGER")
				|| range.asClass().hasSuperClass(expressModel.getOntClass(expressNS + "INTEGER")))
			return "integer";
		else if (range.asClass().getURI().equalsIgnoreCase(expressNS + "BINARY")
				|| range.asClass().hasSuperClass(expressModel.getOntClass(expressNS + "BINARY")))
			return "hexBinary";
		else if (range.asClass().getURI().equalsIgnoreCase(expressNS + "BOOLEAN")
				|| range.asClass().hasSuperClass(expressModel.getOntClass(expressNS + "BOOLEAN")))
			return "boolean";
		else if (range.asClass().getURI().equalsIgnoreCase(expressNS + "LOGICAL")
				|| range.asClass().hasSuperClass(expressModel.getOntClass(expressNS + "LOGICAL")))
			return "logical";
		else if (range.asClass().getURI().equalsIgnoreCase(expressNS + "NUMBER")
				|| range.asClass().hasSuperClass(expressModel.getOntClass(expressNS + "NUMBER")))
			return "double";
		else
			return null;
	}
	...
	private Resource getResource(String uri, OntResource rclass) {
		...
		if (r == null) {
			...
			System.out.println("getResource HERE");
			if (r == null)
				System.out.println("getResource r is null");
			if (rclass == null)
				System.out.println("getResource rclass is null");
			if (ttlWriter == null)
				System.out.println("getResource ttlWriter is null");
			resourceMap.put(uri, r);
			try {
				ttlWriter.triple(new Triple(r.asNode(), RDF.type.asNode(), rclass.asNode()));
			} catch (Exception e) {
				System.err.println("getResource Error");
				e.printStackTrace();
				if (myIfcReaderStream.logToFile)
					try {
